  ...
  AFRAME.registerComponent("sprite-particles", {
    ...
    update(oldData) {
      ...
      if (data.trailLifeTime !== oldData.trailLifeTime) {
        ...
        this.trailLifeTime = parseVecRange(data.trailLifeTime, [
          0,
        ]).map((x, i) => (x > 0 ? x : this.lifeTime[i]));
    ...
    updateAttributes() {
      if (this.geometry) {
        ...
        if (this.data.source) {
          this.geometry.addAttribute(
            "quaternion",
            new THREE.Float32BufferAttribute(new Float32Array(n * 4).fill(0), 4)
          );
        }
    ...
    updateDefines() {
      ...
      const domDefines = domAttrs
        .map((a) => ATTR_TO_DEFINES[a])
        .filter((b) => b);
